...
flash) plus .wav previews of the same audio so you can audition on a laptop.
"""
...
def sub(note=33, dur=0.60):
    n = int(dur * SR); t = np.arange(n) / SR
    f = hz(note) * (1 + 0.35 * np.exp(-t * 40))
    s = np.sin(2 * np.pi * np.cumsum(f) / SR)
    return np.tanh(s * 1.4) * env(n, 0.004, 0, 4.5) * 0.72
...
PADS = [
    ...
    ("06_stab_em",  stab([64, 67, 71])),        # E  G  B
    ("07_sub",      sub(33)),                   # A1
...
def build_loop(bars=4):
    ...
    bassline = [(0, 33), (6, 33), (10, 36), (14, 40)]   # A A C E
...
def demo():
    ...
    presses = [
        ...
        (32, "06_stab_em"), (34, "08_pluck"), (38, "05_stab_c"), (44, "09_riser"),
        (48, "04_stab_am"), (52, "08_pluck"), (54, "08_pluck"), (56, "06_stab_em"),
        (60, "05_stab_c"),  (62, "08_pluck"),
        (64, "04_stab_am"), (68, "08_pluck"), (72, "06_stab_em"), (76, "05_stab_c"),
    ...
    out = np.tanh(buf[:n] * 0.62) * 1.2
    return norm(out)


d = demo()
pcm = (d * 32767).astype("<i2")
with wave.open(os.path.join(OUT, "doorbell-house-demo.wav"), "wb") as w:
    w.setnchannels(1); w.setsampwidth(2); w.setframerate(SR)
    w.writeframes(pcm.tobytes())

with open(os.path.join(OUT, "manifest.txt"), "w") as f:
    f.write("DJ doorbell sample set\n")
    f.write("raw 16-bit PCM mono, little-endian, %d Hz, %.1f BPM, A minor\n\n" % (SR, BPM))
